...
- [x] **Fase 10 — Panel financiero de admin** (`admin.html` → 💰 Finanzas): endpoint `admin_finanzas` con KPIs (ingreso bruto cobrado, ganancia plataforma/comisión, pagado a médicos, en custodia, por cobrar en próximas consultas, reembolsado, ticket promedio), tabla de ingresos por médico (neto ganado, etc.) e ingresos por mes (últimos 6 meses).
- [x] **Fase 9.3 — Especialidades, idiomas y experiencia editables**: especialidades e idiomas pasan a listas (agregar/quitar) en el portal; card nueva "Experiencia profesional" (cargo/institución/período). Columnas JSON `especialidades`/`idiomas_lista`/`experiencia`; `especialidad` (principal) sincronizada con la 1ª; el filtro por especialidad ahora es **match-any** (aparece si cualquiera coincide). Perfil público muestra principal + secundarias (chips) + idiomas + experiencia (💼).
- [x] **Fase 9.2 — Historial educativo dinámico**: card aparte "Historial educativo" en el portal donde el médico agrega/quita entradas (tipo: título universitario, postgrado, especialización, doctorado, maestría, pasantía, curso… + institución, título/detalle, año). Se guarda en `medico_especialidad.educacion` (JSON) y se muestra en el perfil público con íconos por tipo. Migra los antiguos universidad/postgrado.
- [x] **Fase 9.1 — Foto de perfil editable en el portal**: se restauró la opción de subir/cambiar la foto de perfil en "Mi perfil" del portal médico, reutilizando el recortador (drag+zoom → canvas 400×400) del registro. `guardarPerfil` envía `foto_base64` solo si es una foto nueva (dataURL); el backend `medico_actualizar` ya la persistía.
- [x] **Fase 9 — Editar disponibilidad + horarios en el card**: el médico edita su disponibilidad semanal desde **Agenda** (botón "⚙️ Editar mis horarios de atención", grid días×bloques, endpoint dedicado `medico_disponibilidad_guardar`); el card público muestra los **horarios de la semana** agrupados por día (en lugar de solo "próximo disponible"). Fix: orden de días en `listarMedicos` (acentos del enum).
- [x] **Fase 8 — Cancelar / reprogramar citas**: el paciente cancela o reprograma sus citas desde su portal (autoservicio hasta **12 h** antes; reprogramar reusa el selector de horarios y resetea la cita a `agendada`); el médico cancela con motivo (reembolso + email al paciente). Reembolso contable (`reembolsado` + transacción) y liberación del horario. Fix: una cita `confirmada` ahora **ocupa** su slot (antes lo liberaba por error). Cancelar una cita de **cortesía** restituye el uso del código (decrementa `usos_count`, reactiva `agotado→activo`, borra la fila de `codigo_usos`).
